...
RUN apt-get update && apt-get install -y  libcurl4-openssl-dev libicu-dev libssl-dev libv8-dev libxml2-dev make pandoc zlib1g-dev && rm -rf /var/lib/apt/lists/*
...
RUN Rscript -e 'remotes::install_version("fabletools",upgrade="never", version = "0.3.3")'
...
RUN Rscript -e 'remotes::install_version("RTL",upgrade="never", version = "1.3.3")'
